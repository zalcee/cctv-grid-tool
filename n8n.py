...
async def send_to_n8n_webhook(image_path: Path, retention_data: dict, brand: str):
    """Sends the generated collage image and retention data payload to the n8n webhook."""
    if not N8N_WEBHOOK_URL:
        print("ℹ️ N8N_WEBHOOK_URL not configured. Skipping webhook notification.")
        return

    print(f"🚀 Sending payload and snapshot to n8n webhook: {N8N_WEBHOOK_URL}")
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as webhook_client:
            with open(image_path, "rb") as img_file:
                ...
                # Pass structured retention data as form fields/metadata alongside the file
                data = {
                    "brand": brand,
                    "timestamp": datetime.now(PHT).isoformat(),
                    "retention_payload": json.dumps(retention_data)
                }
                ...
                if response.status_code in (200, 201):
                    print("✅ Successfully delivered payload to n8n webhook!")
                else:
                    print(f"⚠️ Webhook responded with status code {response.status_code}: {response.text}")
    except Exception as e:
        print(f"❌ Failed to send data to n8n webhook: {str(e)}")
...
async def generate_collage_and_retention():
    ...
    # Trigger webhook notification after successful file generation
    await send_to_n8n_webhook(image_path, retention_data, brand)
